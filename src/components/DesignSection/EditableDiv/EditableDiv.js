...
import {
  onObjectSnapping,
  onObjectSnappingEnd,
} from "../../../utilities/ObjectSnappingUtil";
...
function EditableDiv(props) {
  ...
  const findAndReplaceElement = (layers) => {
    ...
    for (let i = 0; i < copyLayerList.length; i++) {
      ...
      if (copyLayer.type === "Group") {
        let returnChildElements = findAndReplaceElement(
          copyLayer.childElements
        );
    ...
    return (
      <React.Fragment>
        {childElements.map((element, i) => {
          return (
            (props.groupIndex !== index || props.cropIndex !== i) &&
            getElement(`child_${index}_${i}`, element)
          );
        })}
      </React.Fragment>
    );
  ...
  function getElement(index, element) {
    return (
      !element.hide && (
        <Element
          ...
          setShapeRef={(value) => {
            if (
              ...
              value &&
              value.current &&
              shapeRef.current.getId() !== value.current.getId()
            ) {
              props.setChartData(null);
            }
            setShapeRef(value);
        ...
        >
          {element.type === "Group" &&
            createChildElements(index, element.childElements)}
        </Element>
      )
    );
  }
  ...
  const unGroupTempGroup = (canDeletePermanentGroup) => {
    if (selectedGroupId) {
      props.handleLayerChangeCallback()((prev) => {
        let layers = [...prev];
        let { copyLayers, groupId } = deleteTempGroup(
          layers,
          selectedGroupId,
          stageRef,
          canDeletePermanentGroup,
          props.generateKey
        );
        setGroupId(groupId);
        return copyLayers;
  ...
  const unGroupPermanentGroup = () => {
    if (selectedGroupId) {
      props.handleLayerChangeCallback()((prev) => {
        let changedLayers = deletePermanentGroup(
          prev,
          selectedGroupId,
          stageRef
        );
  ...
  const addImageWhenPaste = (el, parentId) => {
    ...
    if (copyElement.type === "Group") {
      ...
      let key = copyLayer.transperantRect
        ? `BACKGROUND_${parentId}`
        : props.generateKey();
      copyElement.key = key;
      copyElement.id = key;
  ...
  const handleResize = (style, isShiftKey, type) => {
    ...
    if (
      width >
      containerOffset.width - 2 * containerOffset.minHorizontalPadding
    ) {
    ...
    if (
      height >
      containerOffset.height - 2 * containerOffset.minVerticalPadding
    ) {
    ...
    type === "l" || type === "r"
      ? setResizeWidth(width * zoomLevel)
      : setResizeHeight(height * zoomLevel);
  };
  ...
  const resizeHandler = () => {
    let details = getOffsetWidthAndHeight(
      "#editable-div",
      props.stageWidth,
      props.stageHeight
    );
    ...
    <div
      className={
        ...
        (props.selectedElement
          ? "editable-div-wrapper-shrink"
          : "editable-div-wrapper-expand")
      }
    >
      <EditOptions
        layer={props.layer}
        ...
        >
          ...
          )}
          {
            ...
            >
              <Layer
                ...
                onDragEnd={() => {
                  let layer =
                    layerRef && layerRef.current ? layerRef.current : null;
              ...
              >
                {props.layer.map((element, i) => {
                  return (
                    ((!props.groupIndex && props.cropIndex !== i) ||
                      props.groupIndex ||
                      !(onLoadedCropImage && onLoadedCropperImage)) &&
          ...
          {props.cropIndex !== null && (
            ...
            <NewCropImage
              ...
              temporalCropIndex={
                shapeRef && shapeRef.current ? shapeRef.current.attrs.id : 0
              }
            />
          )}
          {onResize && (
            <ResizableRect
              left={Math.max(
                (containerOffset.width - resizeWidth * zoomLevel) / 2,
                containerOffset.minHorizontalPadding
              )}
              top={Math.max(
                (containerOffset.height - resizeHeight * zoomLevel) / 2,
                containerOffset.minVerticalPadding
              )}
